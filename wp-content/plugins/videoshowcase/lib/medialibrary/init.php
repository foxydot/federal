...
if ( ! class_exists( 'IT_Media_Library' ) ) {
	class IT_Media_Library {
		var $_version = '1.0.8'; // Change this in lib/medialibrary/load.php as well.
		var $_pre_wp_3_4_compatibility = false;
		var $_pre_wp_3_5_compatibility = false;
		 ...
		 */
		function IT_Media_Library( &$parent, $options = '' ) {
			if ( !empty( $options ) ) {
			...
			if ( version_compare( $GLOBALS['wp_version'], '3.4.2', '<=' ) )
				$this->_pre_wp_3_5_compatibility = true;
			...
			if ( ( basename( $_SERVER['PHP_SELF'] ) == 'media-upload.php' ) || ( basename( $_SERVER['PHP_SELF'] ) == 'async-upload.php' ) || ( basename( $_SERVER['PHP_SELF'] ) == 'admin-ajax.php' ) ) {
				...
				if ( ! $this->_pre_wp_3_4_compatibility ) {
					...
					else if ( ( isset( $_POST['save'] ) || isset( $_POST['send'] ) ) && isset( $_POST['attachments'] ) ) {
		...
		$buffer = str_replace( '\'savesend\'', '\'savesend\' style=\'display:none;\'', $buffer );
		 ...
		 *	get_add_link()
		 *
		 *	Determines the correct link to open up the Add Image dialog
		 *
		 *	@return	string	Link URL.
		 */
		function get_add_link( $args ) {
			$defaults = array(
				'text'    => __( '+ Add Image', 'it-l10n-videoshowcase' ),
				'classes' => 'button button-primary',
				'styles'  => '',
			);
			$args = wp_parse_args( $args, $defaults );
			$args['styles'] = empty( $args['styles'] ) ? '' : ' style="' . $args['styles'] . '"';

			if ( $this->_pre_wp_3_5_compatibility )
				return '<a href="' . $this->get_link() . '" class="' . esc_attr( $args['classes'] ) . ' thickbox" ' . $args['styles'] . '>' . $args['text'] . '</a>';
			else
				return '<a href="#" class="' . esc_attr( $args['classes'] ) . ' add-new-image" ' . $args['styles'] . '>' . $args['text'] . '</a>';
		}
...
if ( !empty( $medialibrary_options ) ) {
	$this->_medialibrary = new IT_Media_Library( $this, $medialibrary_options );
} else {
	$this->_medialibrary = new IT_Media_Library( $this );
}
*/
?>
