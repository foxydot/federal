...
include_once('includes/single-site-tweaks.php');
...
function msdlab_permalink($guid){
    $blog_domain = preg_match('/http\:\/\/(.*?)\/(.*?)\?(.*?)(\d+?)$/i',$guid,$matches);
    $the_blog = get_blog_details(array('domain'=>$matches[1]));
    $pl = get_blog_permalink($the_blog->blog_id, $matches[4]);
    return $pl;
}
/**
...
function msd_case_study_excerpt($post){
...
function add_styles() {
    wp_enqueue_style( $sub.'-style', get_bloginfo('template_url').'/css/federal-style.css', FALSE, '0.1', 'all' );
